...
TAGO_MQTT_PORT = 1883
TAGO_MQTT_USERNAME = "HydroSystem - PoC"
TAGO_MQTT_PASSWORD = "c6111cd9-91b6-4a2d-9d6e-7f05810d25e5"
TAGO_DEVICE_TOKEN = "c6111cd9-91b6-4a2d-9d6e-7f05810d25e5"
...
while True:
    ...
    if time_now[HOURS] >= DAYTIME["min"] and time_now[HOURS] <= DAYTIME["max"]:
        ...
        if next_time_off["valid"]:
            if(time_now[HOURS] is next_time_off["hours"]) and (time_now[MINUTES] is next_time_off["minutes"]):
                ...
                next_time_on = calc_next_time(next_time_on, next_time_off, 0, 10)
        ...
        if next_time_off["valid"]:
            if(time_now[HOURS] is next_time_off["hours"]) and (time_now[MINUTES] is next_time_off["minutes"]):
                relays.turn_output(PUMP_OUT, TURN_OFF)
                next_time_on = calc_next_time(next_time_on, next_time_off, 0, 20)
                next_time_off["valid"] = False
    else:
        # Deu ruim...
        relays.turn_output(PUMP_OUT, TURN_OFF)
    ...
    # Checando por do sol
    if time_now[HOURS] >= 18 and time_now[HOURS] <= 22:
        if not is_sunset:
            relays.turn_output(LAMP_OUT, TURN_ON)
            is_sunset = True
    if is_sunset and (time_now[HOURS] >= 22 and time_now[MINUTES] >= 00):
    ...
    if next_time_send_data["valid"] and time_now[HOURS] is next_time_send_data["hours"] and time_now[MINUTES] is next_time_send_data["minutes"]:
        ...
        calc_next_time(next_time_send_data, next_time_send_data, 0, 10)
        wifi_config.disconnect()
    sleep(1)
